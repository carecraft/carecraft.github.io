...
Socket起源于“万般皆文本”的Unix，是“open—write/read—close”模式的一种实现，Linux系统
...
既然是特殊的文件，必然支持“open—write/read—close”操作。以TCP为例，介绍socket几个常
用操作。
...
write函数将buf中的count字节内容写入文件描述符fd。成功时返回写的字节数,失败时返回-1，并设置errno变量。
 在网络程序中，当我们向套接字文件描述符写时有两种可能。1)write的返回值大于0，表示写了部分或者是全部的数据。2)返回的值小于0，此时出现了错误。我们要根据错误类型来处理。如果错误为EINTR表示在写的时候出现了中断错误。如果为EPIPE表示网络连接出现了问题(对方已经关闭了连接)。
